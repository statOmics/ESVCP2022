...
when $\epsilon_i \sim N(0,\sigma^2)$

$$ t = \frac{
\hat \beta_1
}{\text{se}_{\hat{\beta}_1}}
$$

which follows a t-distribution with $n-2$ DF if the variance has to be estimated from the data. 

## Critical difference

Assumptions under the null hypothesis

$$Y_1, Y_2 \stackrel{i.i.d.}{\sim} N(\mu, \sigma^2)$$
$$\frac{\sigma}{\mu} = \sqrt{CV_a^2 + CV_i^2}$$
$$ z =  \frac{Y_2-Y_1}{\text{se}_{Y_2-Y_1}}=\frac{Y_2-Y_1}{\sqrt{2  \sigma^2}}=\frac{Y_2-Y_1}{ \sqrt{2} \mu\sqrt{CV^2_a+CV^2_i}} \stackrel{H_0}{\sim} N(0,1)$$
If $\mu$, $CV^2_a$ and $CV^2_i$ are known. 
